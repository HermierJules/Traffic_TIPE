...
let graph_import_node file_node file_edges= 
	...
	for i = 1 to Array.length edges do
		...
		x:= (match Hashtbl.find_opt og_id edges.(i).(1)  with
			 ...
			 |None -> failwith "pas de node originelle");
		...
		for j = 1 to !l do
			vtx:= !node_nb::vtx;
			if !l <> 1 
				then if j <> 1 then ed_temp:= (!nb_node, !nb_node + 1):: ed_temp
							   else ed_temp:= (!x, !nb_node + 1):: ed_temp
				else if j <> 1 then ed_temp:= (!nb_node, !y) :: ed_temp
		...
		l:= ((int_of_float (float_of_string edges.(i).(3))) / 5) + 1;
		if edges.(i).(6) <> 0 then begin
		for j = 1 to !l do
			vtx:= !node_nb::vtx;
			if !l <> 1 
				then if j <> 1 then ed_temp:= (!nb_node, !nb_node + 1):: ed_temp
							   else ed_temp:= (!x, !nb_node + 1):: ed_temp
				else if j <> 1 then ed_temp:= (!nb_node, !x) :: ed_temp
